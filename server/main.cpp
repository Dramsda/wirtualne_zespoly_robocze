...
#include <vector>
#define JOIN_REQUEST 67
#define VOTE_ACCEPT 1
#define VOTE_ASK 2
#define ALLOW_JOIN 4
#define DISALLOW_JOIN 5
	...
	int       nCmdShow)
{
	...
	unicast_net* reciv = new unicast_net(67);
	unicast_net* send = new unicast_net(66);

	Frame frame;
	unsigned long senderIp = 0;

	for (;;) {
		int frame_size = reciv->reciv((char*)&frame, &senderIp, sizeof(Frame));
		//printf("Odebrano IP: %ld\n", senderIp);
		//printf("Type: %d\n", frame.type);
		//printf("ID: %d\n", frame.iID);
		if (frame.type == JOIN_REQUEST) {
			
			// If first player
			if (ingame.size() == 0) {
				printf("This is the first player %d -> ALLOWED\n\n", frame.iID);
				ingame.push_back(senderIp);
				idToIp[frame.iID] = senderIp;
				frame.type = ALLOW_JOIN;
				frame.iID = 0;
				send->send((char*)&frame, senderIp, sizeof(Frame));
				continue;
			}

			// If next player, ask every player ingame
			printf("This is the NOT the first player %d -> ASKING OTHERS\n\n", frame.iID);
			idToIp[frame.iID] = senderIp;
			glosy[frame.iID] = 0;
			frame.id_pytajacego = frame.iID;
			frame.type = VOTE_ASK;
			frame.iID = 0;
			
			for (auto& ip : ingame) {
				send->send((char*)&frame, ip, sizeof(Frame));
			}
		}
		if (frame.type == VOTE_ACCEPT) {
			// Jezeli acccept -> + glos
			glosy[frame.id_pytajacego]++;
			printf("VOTE ACCEPTED FOR %d, current votes: %d\n\n", frame.id_pytajacego, glosy[frame.id_pytajacego]);

			// Wpusc jezeli 1 glos
			if (glosy[frame.id_pytajacego] == 1) {
				printf("ALLOWED TO JOIN %d\n\n", frame.id_pytajacego);
				// Zmapuj id tego ktory chce dolaczyc na ip
				long ip = idToIp[frame.id_pytajacego];

				frame.iID_receiver = frame.id_pytajacego;
				frame.type = ALLOW_JOIN;
				frame.iID = 0;
				send->send((char*)&frame, ip, sizeof(Frame));
			}
